...
shaka.dash.DashParser = class {
  ...
  parsePeriod_(context, baseUris, periodInfo) {
    ...
    const closedCaptionsTextStreams =
      this.createTextStreamsForClosedCaptions_(videoStreams);

    textStreams.push(...closedCaptionsTextStreams);
   ...
   * @return {!Array.<!shaka.extern.Stream>} The closed caption text streams
   * @private
   */
  createTextStreamsForClosedCaptions_(videoStreams) {
    ...
    for (const videoStream of videoStreams) {
      if (videoStream.closedCaptions) {
        for (const id of videoStream.closedCaptions.keys()) {
          if (!closedCaptionsMap.has(id)) {
            const textStream = {
              id: this.globalId_++,  // A globally unique ID.
              originalId: id, // The CC ID string, like 'CC1', 'CC3', etc.
              createSegmentIndex: () => Promise.resolve(),
              // The captions are embedded.
              ...
              drmInfos: [],
              keyIds: new Set(),
              language: videoStream.closedCaptions.get(id),
              label: null,
              type: shaka.util.ManifestParserUtils.ContentType.TEXT,
              primary: false,
              trickModeVideo: null,
              emsgSchemeIdUris: null,
              roles: videoStream.roles,
              channelsCount: null,
    ...
    return Array.from(closedCaptionsMap.values());
  }
